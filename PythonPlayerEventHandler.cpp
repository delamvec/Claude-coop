...
void CPythonPlayerEventHandler::OnHit(UINT uSkill, CActorInstance& rkActorVictim, BOOL isSendPacket, BlendingPosition* pBlending)
{
...
if (isSendPacket)
{
    ...
    {
        blend.source = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
        blend.dest = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
        blend.duration = 0.0f;
    }
...
void CPythonPlayerEventHandler::CNormalBowAttack_FlyEventHandler_AutoClear::OnExplodingAtAnotherTarget(DWORD dwSkillIndex, DWORD dwVID)
{
	...
	// Vytvoř BlendingPosition pro SendAttackPacket
	BlendingPosition blend;
	blend.source = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	blend.dest = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	blend.duration = 0.0f;

	rkStream.SendAttackPacket(dwSkillIndex, dwVID, TRUE, blend);  // ✅ 4 parametry
