...
bool CPythonNetworkStream::RecvCharacterAttackPacket()
{
    ...
    TraceError("[GC_ATTACK_RECV_RAW] Type:%d AttackerVID:%d VictimVID:%d Packet:%d SrcPos:(%d,%d) DstPos:(%d,%d) SyncDest:(%.2f,%.2f) BlendDur:%dms",
        ...
        kPacket.fSyncDestX, kPacket.fSyncDestY,
        kPacket.dwBlendDuration
    );
...
bool CPythonNetworkStream::SendAttackPacket(UINT uMotAttack, DWORD dwVIDVictim, BOOL bPacket, BlendingPosition& sBlending)
{
    ...
    TraceError("[CG_ATTACK_SEND] Type:%d VictimVID:%d Packet:%d SrcPos:(%d,%d) DstPos:(%d,%d) SyncDest:(%.2f,%.2f) BlendDur:%dms Combo:%d Time:%d",
        ...
        kPacketAtk.dwVID,
        kPacketAtk.bPacket,
        kPacketAtk.lSX, kPacketAtk.lSY,
        kPacketAtk.lX, kPacketAtk.lY,
        kPacketAtk.fSyncDestX, kPacketAtk.fSyncDestY,
        kPacketAtk.dwBlendDuration,
        kPacketAtk.dwComboMotion,
        kPacketAtk.dwTime
    );
