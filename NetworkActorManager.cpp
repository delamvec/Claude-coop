...
void SNetworkActorData::UpdatePosition()
{
	...
	if (dwElapsedTime<m_dwDuration)
	{
		...
		if (m_dwDuration == 0)
		{
			TraceError("SNetworkActorData::UpdatePosition - Duration is zero, cannot calculate position rate");
			m_lCurX=m_lDstX;
			m_lCurY=m_lDstY;
			return;
		}
...
{
	// Add validation for VID - but only log, don't block (VID 0 might be valid in some cases)
	if (rkNetActorData.m_dwVID == 0)
	{
		Tracenf("CNetworkActorManager::__AppendCharacterManagerActor - Warning: VID is 0");
	}

	// Log if name is empty but allow creation (some NPCs might not have names)
	if (rkNetActorData.m_stName.empty())
	{
		Tracenf("CNetworkActorManager::__AppendCharacterManagerActor - Empty actor name for VID %d", rkNetActorData.m_dwVID);
	}
