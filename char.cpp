...
void CHARACTER::StateSyncing()
{
    ...
    int x = (int) ((float) (m_posBlendDest.x - m_posBlendStart.x) * fRate + m_posBlendStart.x);
    int y = (int) ((float) (m_posBlendDest.y - m_posBlendStart.y) * fRate + m_posBlendStart.y);
...
bool CHARACTER::BlendSync(long x, long y, unsigned int unDuration)
{
    ...
    m_posBlendDest.x = m_posBlendStart.x = GetX();
    m_posBlendDest.y = m_posBlendStart.y = GetY();
    ...
    GotoConcurrentState(m_stateSyncing);
    return true;
}
