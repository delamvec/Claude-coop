...
void CActorInstance::__OnHit(UINT uSkill, CActorInstance& rkActorVictm, BOOL isSendPacket, BlendingPosition* pBlending)
{
    IEventHandler& rkEventHandler=__GetEventHandlerRef();
    rkEventHandler.OnHit(uSkill, rkActorVictm, isSendPacket, pBlending);
}
...
CActorInstance::IEventHandler* CActorInstance::IEventHandler::GetEmptyPtr()
{
	static class CEmptyEventHandler : public IEventHandler
	{
		public:
			...
			virtual void OnHit(UINT uMotAttack, CActorInstance& rkActorVictim, BOOL isSendPacket, BlendingPosition* pBlending) {}
