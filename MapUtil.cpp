...
bool Environment_Load(SEnvironmentData& envData, const char* envFileName)
{
	...
	if (!envFileName || envFileName[0] == '\0')
	{
		TraceError("Environment_Load - Invalid environment file name (null or empty)");
		return false;
	}
	...
	{
		TraceError("Environment_Load - Failed to load environment file: %s", envFileName);
		return false;
	}
...
void PixelPositionToAttributeCellPosition(TPixelPosition PixelPosition, TCellPosition * pAttrCellPosition)
{
	...
	if (!pAttrCellPosition)
	{
		TraceError("PixelPositionToAttributeCellPosition - Null output pointer provided");
		return;
	}

	// Add validation for cell size to prevent division by zero
	if (c_Section_xAttributeCellSize == 0 || c_Section_yAttributeCellSize == 0)
	{
		TraceError("PixelPositionToAttributeCellPosition - Invalid attribute cell size (zero division)");
		pAttrCellPosition->x = 0;
		pAttrCellPosition->y = 0;
		return;
	}
...
void AttributeCellPositionToPixelPosition(TCellPosition AttrCellPosition, TPixelPosition * pPixelPosition)
{
	...
	if (!pPixelPosition)
	{
		TraceError("AttributeCellPositionToPixelPosition - Null output pointer provided");
		return;
	}
...
float CEaseOutInterpolation::GetRemainingTime() const
{
...
{
	// Add validation for time parameter
	if( fabs(fTime) < FLT_EPSILON )
	{
		TraceError("CEaseOutInterpolation::Setup - Time value too small (%f), adjusting to 0.01f", fTime);
		fTime = 0.01f;
	}

	// Add validation for extreme values
	if (!isfinite(fStart) || !isfinite(fEnd))
	{
		TraceError("CEaseOutInterpolation::Setup - Invalid start (%f) or end (%f) value (not finite)", fStart, fEnd);
		return FALSE;
	}
...
BOOL CEaseOutInterpolation::isPlaying() const  // ← přidej const
{
	return m_fRemainingTime > 0.0f;
}

float CEaseOutInterpolation::GetValue() const  // ← přidej const
{
	return m_fValue;
}

float CEaseOutInterpolation::GetChangingValue() const  // ← přidej const
{
