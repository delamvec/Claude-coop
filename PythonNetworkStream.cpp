...
#include "../GameLib/BlendingPosition.h"
...
class CMainPacketHeaderMap : public CNetworkPacketHeaderMap
{
	...
	public:
		CMainPacketHeaderMap()
		{
			...
			Set(HEADER_GC_ATTACK,				CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCAttack), STATIC_SIZE_PACKET));
...
void CPythonNetworkStream::SetPhaseWindow(UINT ePhaseWnd, PyObject* poPhaseWnd)
{
	...
	{
		TraceError("CPythonNetworkStream::SetPhaseWindow - Invalid phase window index: %d (max: %d)", ePhaseWnd, PHASE_WINDOW_NUM-1);
		return;
	}

	if (!poPhaseWnd)
	{
		TraceError("CPythonNetworkStream::SetPhaseWindow - Null phase window object for index: %d", ePhaseWnd);
		return;
	}
...
void CPythonNetworkStream::ClearPhaseWindow(UINT ePhaseWnd, PyObject* poPhaseWnd)
{
	if (ePhaseWnd>=PHASE_WINDOW_NUM)
	{
		TraceError("CPythonNetworkStream::ClearPhaseWindow - Invalid phase window index: %d (max: %d)", ePhaseWnd, PHASE_WINDOW_NUM-1);
		return;
	}

	if (!poPhaseWnd)
	{
		TraceError("CPythonNetworkStream::ClearPhaseWindow - Null phase window object for index: %d", ePhaseWnd);
		return;
	}

	if (poPhaseWnd != m_apoPhaseWnd[ePhaseWnd])
	{
		TraceError("CPythonNetworkStream::ClearPhaseWindow - Phase window mismatch for index: %d", ePhaseWnd);
		return;
	}
...
UINT CPythonNetworkStream::GetAccountCharacterSlotDatau(UINT iSlot, UINT eType)
{
	...
	{
		TraceError("CPythonNetworkStream::GetAccountCharacterSlotDatau - Invalid slot index: %d (max: %d)", iSlot, PLAYER_PER_ACCOUNT4-1);
		return 0;
	}
...
const char* CPythonNetworkStream::GetAccountCharacterSlotDataz(UINT iSlot, UINT eType)
{
	...
	if (iSlot >= PLAYER_PER_ACCOUNT4)
	{
		TraceError("CPythonNetworkStream::GetAccountCharacterSlotDataz - Invalid slot index: %d (max: %d)", iSlot, PLAYER_PER_ACCOUNT4-1);
		return sc_szEmpty;
	}
...
void CPythonNetworkStream::ConnectGameServer(UINT iChrSlot)
{
	...
	{
		TraceError("CPythonNetworkStream::ConnectGameServer - Invalid character slot: %d (max: %d)", iChrSlot, PLAYER_PER_ACCOUNT4-1);
		return;
	}
	...
	if (rkSimplePlayerInfo.lAddr == 0)
	{
		TraceError("CPythonNetworkStream::ConnectGameServer - Invalid server address for slot: %d", iChrSlot);
		return;
	}

	if (rkSimplePlayerInfo.wPort == 0)
	{
		TraceError("CPythonNetworkStream::ConnectGameServer - Invalid server port for slot: %d", iChrSlot);
		return;
	}
