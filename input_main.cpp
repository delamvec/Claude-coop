		...
		case HEADER_CG_ATTACK:
			{
				...
				// VALIDACE A SYNCHRONIZACE POZICE
				if (packMelee->fSyncDestX != 0.0f && packMelee->fSyncDestY != 0.0f)
				{
					// Kontrola vzdálenosti od sync pozice
					float fDistFromSync = DISTANCE_SQRT(ch->GetX() - (long)packMelee->fSyncDestX, 
													ch->GetY() - (long)packMelee->fSyncDestY);
					
					// Pokud je hráč příliš daleko od své deklarované pozice, odmítnout útok
					if (fDistFromSync > 500) // Tolerance 500 jednotek
					{
						sys_log(0, "Attack rejected: %s too far from sync position (%.1f)", 
								ch->GetName(), fDistFromSync);
						return;
					}
					
					// SPUSTIT BLEND SYNCHRONIZACI PŘED ÚTOKEM
					if (packMelee->dwBlendDuration > 0)
					{
						ch->BlendSync((long)packMelee->fSyncDestX, 
									(long)packMelee->fSyncDestY, 
									packMelee->dwBlendDuration);
					}
				}

				// KONTROLA SKILL HIT COUNT
				...
				// PROVÉST ÚTOK (BlendSync už běží na pozadí v concurrent state)
				ch->Attack(victim, packMelee->bType);

				// ODESLAT GC PACKET VŠEM HRÁČŮM V OKOLÍ
				TPacketGCAttack pack;
				pack.bHeader = HEADER_GC_ATTACK;
				pack.bType = packMelee->bType;
				pack.dwAttacakerVID = ch->GetVID();
