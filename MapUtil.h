...
class CEaseOutInterpolation
{
	public:
		...
		BOOL isPlaying() const;  // ← PŘIDÁNO const

		float GetValue() const;  // ← PŘIDÁNO const
		float GetChangingValue() const;  // ← PŘIDÁNO const
		float GetRemainingTime() const;  // ← PŘIDÁNO const (nová metoda, pokud ještě neexistuje)
