...
float CInstanceBase::GetBlendingRemainTime()
{
    if (!m_GraphicThingInstance.IsPositionBlending())
        return 0.0f;
        
    DWORD dwCurrentTime = ELTimer_GetMSec();
    DWORD dwElapsedTime = dwCurrentTime - m_GraphicThingInstance.GetBlendStartTime();  // ✅ getter
    
    if (dwElapsedTime >= m_GraphicThingInstance.GetBlendDuration())  // ✅ getter
        return 0.0f;
        
    DWORD dwRemainMS = m_GraphicThingInstance.GetBlendDuration() - dwElapsedTime;  // ✅ getter
    return float(dwRemainMS) / 1000.0f;
}
